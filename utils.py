...
def load_mat(dataset, train_rate=0.6, val_rate=0.1):

    """Load .mat dataset."""
    data = sio.loadmat("./dataset/{}.mat".format(dataset))
    label = data['Label'] if ('Label' in data) else (data['label'] if ('label' in data) else data['gnd'])
    attr = data['Attributes'] if ('Attributes' in data) else (data['features'] if ('features' in data) else data['X'])
    network = data['Network'] if ('Network' in data) else (data['homo'] if ('homo' in data) else data['A'])
    ...
    # Build a labeled anomaly pool from true anomalous nodes inside the training set.
    real_abnormal_idx = [i for i in idx_train if ano_labels[i] == 1]
    if not real_abnormal_idx:
        # Fallback to global anomalies if the training split has none (rare edge case)
        real_abnormal_idx = [i for i in all_idx if ano_labels[i] == 1]
    random.shuffle(real_abnormal_idx)

    if dataset in ['Amazon']:
        abn_rate = 0.05
    else:
        abn_rate = 0.15
    target_abn = max(1, int(len(normal_label_idx) * abn_rate))
    abnormal_label_idx = real_abnormal_idx[:min(target_abn, len(real_abnormal_idx))]
