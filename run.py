...
identity = sp.eye(raw_adj.shape[0])
print(raw_adj.sum())

# Propagation adjacency (GCN)
adj_prop = normalize_adj(raw_adj + identity).todense()

# Residual adjacency: remove self-loops, row-normalize
...
with tqdm(total=args.num_epoch) as pbar:
    ...
    for epoch in range(args.num_epoch):
        ...
        # Train model
        train_flag = True
        emb, emb_combine, logits, emb_con, emb_abnormal, _, _, _, _, _ = model(
            features, adj_prop, adj_resid,
            normal_label_idx, normal_label_idx, abnormal_label_idx,
            train_flag, args)
        if epoch % 10 == 0:
            # save data for tsne
            pass
        ...
        if emb_abnormal is not None:
            num_real_abn = emb_abnormal.size(1)
            if num_real_abn > 0:
                label_chunks.append(torch.ones(num_real_abn, device=device))

        lbl = torch.cat(label_chunks, dim=0).unsqueeze(0).unsqueeze(-1)
        ...
        if emb_abnormal is not None and emb_abnormal.size(1) > 0:
            fake_num = emb_con.size(0)
            real_abn = emb_abnormal.squeeze(0)
            real_num = real_abn.size(0)
            if real_num >= fake_num:
                sampled = real_abn[:fake_num]
            else:
                repeat = (fake_num + real_num - 1) // real_num
                sampled = real_abn.repeat(repeat, 1)[:fake_num]
            diff_attribute = torch.pow(emb_con - sampled, 2)
        ...
        if epoch % 2 == 0:
            logits = np.squeeze(logits.detach().cpu().numpy())
            lbl_np = np.squeeze(lbl.detach().cpu().numpy())
            auc = roc_auc_score(lbl_np, logits)
        ...
        if epoch % 10 == 0:
            ...
            with torch.no_grad():
                ...
                idx_test_tensor = torch.tensor(idx_test, device=device, dtype=torch.long)
                logits_test = logits[:, idx_test_tensor, :]
                logits_test_np = np.squeeze(logits_test.detach().cpu().numpy())
                auc = roc_auc_score(ano_label[idx_test], logits_test_np)
                print('Testing {} AUC:{:.4f}'.format(args.dataset, auc))
                AP = average_precision_score(ano_label[idx_test], logits_test_np, average='macro', pos_label=1, sample_weight=None)
                print('Testing AP:', AP)
